...
脚本通过本仓库 `runtime/` 的远程地址加载。重写名称使用源插件的应用名和功能标签，例如“微博 · 开屏广告 preload”，不再显示统一的 `loon-` 或 `ssrules-` 前缀。不同功能名称可以引用同一个脚本文件。旧哈希脚本保留以兼容未更新客户端，但新版主文件只引用当前 8 份脚本，不会加载未引用的旧文件。
