...
MITM 列表共 **132 项**，包括 Loon 主配置的 **5 条排除项**，排除项优先。普通拦截、原生重写和脚本对应的解密列表均已合并去重。高德、微博的 3 个宽泛通配域名已替换为规则实际使用的 12 个接口域名；条目变多，但覆盖的流量范围变小。构建时自动枚举接口；遇到不能可靠枚举的新表达式则保留来源通配域名，避免漏掉功能。
...
需要读取正文的脚本上限已从 1 MiB 降到 **256 KiB**，超出上限不会执行该脚本；执行超时上限为 10 秒。较大的响应可能不再净化。该限制不是脚本总内存或并发内存上限，也不作用于原生正文重写。脚本外置只减少配置体积，不消除运行开销。

用户实测不开覆写时约 20 MB，开启覆写后内存超限。此次调整针对解密范围及正文处理量，保留全部功能配置。尚未取得调整后的手机复测结果；不能据此保证已经解决超限，尤其是脚本加载阶段的开销不会因正文上限降低而消失。
