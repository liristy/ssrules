...
只需导入项目根目录的 `stash_plugins.stoverride`。主文件保留开屏去广告（含叮咚买菜）、YouTube 去广告、苹果天气增强，以及盒马净化和通用广告平台网络拦截。QUIC 分流由主配置转换脚本 `stash_override.js` 管理。

## 当前范围

主文件约 28 KB，包含 455 条分流规则、26 条 URL 拦截/跳转、2 条原生正文重写、1 条静态响应，以及 8 份 JavaScript 的 14 条触发规则。MITM 共 40 项，原有 5 条排除项优先。
...
脚本通过本仓库 `runtime/` 的远程地址加载。重写名称使用源插件的应用名和功能标签，例如“微博 · 开屏广告 preload”，不再显示统一的 `loon-` 或 `ssrules-` 前缀。当前脚本文件名及引用地址也已去掉 `loon-` 前缀。不同功能名称可以引用同一个脚本文件。旧哈希脚本（含旧前缀文件）保留以兼容未更新客户端，但新版主文件只引用当前 8 份脚本，不会加载未引用的旧文件。
